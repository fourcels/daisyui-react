...
export type { ModalHeaderProps, ModalBodyProps, ModalActionProps, ModalActionButtonProps }
        ...
        <ModalContext.Provider value={dialogRef}>
            ...
            >
                <div data-theme={dataTheme} className={bodyClasses}>
                    {close && (
                        <Button onClick={() => dialogRef.current?.close()} size='sm' color='ghost' shape='circle' className='absolute right-2 top-2'>✕</Button>
                    )}
                    {children}
                </div>
                {backdrop && (
